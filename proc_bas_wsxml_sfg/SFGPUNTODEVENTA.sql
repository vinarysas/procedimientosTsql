  ...
  SET NOCOUNT ON;
    ...
    END 
      ...
      SELECT PDV.ID_PUNTODEVENTA,
        ...
        LEFT OUTER JOIN WSXML_SFG.RAZONSOCIAL C
          ON (PDV.CODRAZONSOCIAL = C.ID_RAZONSOCIAL)
        LEFT OUTER JOIN WSXML_SFG.AGRUPACIONPUNTODEVENTA A
          ON (PDV.CODAGRUPACIONPUNTODEVENTA = A.ID_AGRUPACIONPUNTODEVENTA)
        LEFT OUTER JOIN WSXML_SFG.CIUDAD CD
          ON (PDV.CODCIUDAD = CD.ID_CIUDAD)
        LEFT OUTER JOIN WSXML_SFG.DEPARTAMENTO DP
          ON (CD.CODDEPARTAMENTO = DP.ID_DEPARTAMENTO)
        LEFT OUTER JOIN WSXML_SFG.PUERTOTERMINAL PT
          ON (PDV.CODPUERTOTERMINAL = PT.ID_PUERTOTERMINAL)
        LEFT OUTER JOIN WSXML_SFG.RUTAPDV RP
          ON (PDV.CODRUTAPDV = RP.ID_RUTAPDV)
        LEFT OUTER JOIN WSXML_SFG.REDPDV RD
          ON (PDV.CODREDPDV = RD.ID_REDPDV)
        LEFT OUTER JOIN WSXML_SFG.REGIONAL RG
...
CREATE     PROCEDURE WSXML_SFG.SFGPUNTODEVENTA_GetList(@p_active NUMERIC(22,0)) AS
  ...
  SET NOCOUNT ON;
    ...
    -- returned along the row data to support concurrency.
        ...
        LEFT OUTER JOIN WSXML_SFG.DEPARTAMENTO DEPTO   ON (CD.CODDEPARTAMENTO = DEPTO.ID_DEPARTAMENTO)
        LEFT OUTER JOIN WSXML_SFG.TIPONEGOCIO TN       ON (PDV.CODTIPONEGOCIO = TN.ID_TIPONEGOCIO)
        LEFT OUTER JOIN WSXML_SFG.REGIONAL RG       ON (PDV.CODREGIONAL = RG.ID_REGIONAL)
...
CREATE     PROCEDURE WSXML_SFG.SFGPUNTODEVENTA_GetListByGroup(@p_ACTIVE          NUMERIC(22,0),
  ...
  SET NOCOUNT ON;
      ...
      SELECT PUNTODEVENTA.ID_PUNTODEVENTA,
        ...
        LEFT OUTER JOIN WSXML_SFG.DEPARTAMENTO
