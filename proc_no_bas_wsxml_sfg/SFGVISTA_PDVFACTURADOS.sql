...
CREATE     PROCEDURE WSXML_SFG.SFGVISTA_PDVFACTURADOS_GetPDVFacturados( -- NECESARIOS SON ID
 ...
 BEGIN
      ...
      SET NOCOUNT ON;
       ...
       SET @STR_QUERY_INT = ISNULL(@STR_QUERY_INT, '') +
               'SUM(PDV.ACTIVE) AS PDV_REGISTRADO,
				...
				ROW_NUMBER() OVER(ORDER BY CFP.ID_CICLOFACTURACIONPDV ASC) ROWNUM
           ...
           IF @P_RED IS NOT NULL AND @P_RED <> -1 BEGIN
                 SET @STR_QUERY_INT = ISNULL(@STR_QUERY_INT, '') + ' AND PDV.CODREDPDV = ' + ISNULL(CONVERT(VARCHAR,@P_RED), '') ;
           END 

           IF @P_PDV IS NOT NULL AND @P_PDV <> -1 BEGIN
                 SET @STR_QUERY_INT = ISNULL(@STR_QUERY_INT, '') + ' AND PDV.ID_PUNTODEVENTA = ' + ISNULL(CONVERT(VARCHAR,@P_PDV), '') ;
           END 

           SET @STR_QUERY_INT = ISNULL(@STR_QUERY_INT, '') + '
           INNER  JOIN WSXML_SFG.REGISTROFACTURACION REG
                ON  REG.CODPUNTODEVENTA = MFP.CODPUNTODEVENTA
                AND WSXML_SFG.LINEADENEGOCIO_PRODUCTO_F(REG.CODPRODUCTO) = MFP.CODLINEADENEGOCIO
                AND REG.CODTIPOREGISTRO <> 5';
           IF @P_LINEA IS NOT NULL AND @P_LINEA <> -1 BEGIN
                 SET @STR_QUERY_INT = ISNULL(@STR_QUERY_INT, '') + ' AND MFP.CODLINEADENEGOCIO = ' + ISNULL(CONVERT(VARCHAR,@P_LINEA), '') ;
           END 
           ...
           IF @P_CICLO IS NOT NULL AND @P_CICLO <> -1 BEGIN
                 SET @STR_QUERY_INT = ISNULL(@STR_QUERY_INT, '') + ' AND CFP.ID_CICLOFACTURACIONPDV = ' + ISNULL(CONVERT(VARCHAR,@P_CICLO), '') ;
           END 
           ...
           IF @P_CADENA IS NOT NULL AND @P_CADENA <> -1 BEGIN
                 SET @STR_QUERY_INT = ISNULL(@STR_QUERY_INT, '') + ' AND AGR.ID_AGRUPACIONPUNTODEVENTA = ' + ISNULL(CONVERT(VARCHAR,@P_CADENA), '') ;
           END 
           ...
           IF @P_PRODUCTO IS NOT NULL AND @P_PRODUCTO <> -1 BEGIN
                 SET @STR_QUERY_INT = ISNULL(@STR_QUERY_INT, '') + ' AND PRD.ID_PRODUCTO = ' + ISNULL(CONVERT(VARCHAR,@P_PRODUCTO), '') ;
           END 

           IF @P_TIPO_PRD IS NOT NULL AND @P_TIPO_PRD <> -1 BEGIN
                 SET @STR_QUERY_INT = ISNULL(@STR_QUERY_INT, '') + ' AND PRD.CODTIPOPRODUCTO = ' + ISNULL(CONVERT(VARCHAR,@P_TIPO_PRD), '') ;
           END 
      ...
      SET @STR_QUERY_INT_SIZE = 'SELECT COUNT(1) FROM ( ' + ISNULL(@STR_QUERY_INT, '') + ') T';
      ...
      FROM ( SELECT QUERY.*, ROW_NUMBER() OVER(ORDER BY ID_CICLOFACTURACIONPDV ASC) r
             ...
             WHERE ROWNUM <= '+ ISNULL(CONVERT(VARCHAR,@HigerBound), '') +') T
      WHERE r >= '+ISNULL(CONVERT(VARCHAR,@LowerBound), '');
	  ...
	  BEGIN TRY
		...
		EXECUTE sp_executesql @STR_QUERY_OUT
	  END TRY
	  BEGIN CATCH
		PRINT @STR_QUERY_OUT;
	  END CATCH
