...
TABLE (
 ...
 BEGIN
   ...
   --EXEC WSXML_SFG.SFG_PACKAGE_GetMonthRange @p_DATE, @sFECHAFRST OUT, @sFECHALAST OUT
    SELECT @sFECHAFRST = FRSTDATE, @sFECHALAST = LASTDATE FROM WSXML_SFG.SFG_PACKAGE_GetMonthRange_F(@p_DATE) ;
    ...
    IF (SELECT COUNT(*) FROM @lstCICLOSFACTURACION) = 0 BEGIN
		INSERT INTO @lstCICLOSFACTURACION VALUES (NULL, NULL, NULL, CAST('-20085 No existen ciclos de facturacion a la fecha estipulada' AS INT))
		--RAISERROR('-20085 No existen ciclos de facturacion a la fecha estipulada',16,1);
    END 
    /* Sobreescribir fechas para cada ciclo, y obtener fechas generales de lista */
    DECLARE ix CURSOR FOR 
		...
		DECLARE @TOTALFILAS NUMERIC(38,0) = (SELECT COUNT(*) FROM @lstCICLOSFACTURACION)
		...
		BEGIN
			...
			SELECT @iniDATE = INIDATE, @endDATE = ENDDATE  FROM WSXML_SFG.SFGCICLOFACTURACIONPDV_GetCicloDateRangeFromID_F(@ix__ID_CICLOFACTURACIONPDV)
