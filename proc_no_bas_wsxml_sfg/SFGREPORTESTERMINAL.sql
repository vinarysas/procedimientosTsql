...
CREATE PROCEDURE WSXML_SFG.SFGREPORTESTERMINAL_ExpectedCycleID(
  @p_SECUENCIACICLO NUMERIC(22,0), 
  @p_CODCICLOFACTURACIONPDV NUMERIC(22,0) OUT, 
  @p_FECHAULTIMAEJECUCION DATETIME OUT
) AS
BEGIN

  DECLARE @cMAXSECUENCIAFACTURACION NUMERIC(22,0);
  DECLARE @COUNT_REGISTROS NUMERIC(22,0);
  ...
  BEGIN
    ...
    BEGIN
      BEGIN TRY
        ...
        SET @COUNT_REGISTROS = 0
        
        SELECT 
          @p_FECHAULTIMAEJECUCION = ISNULL(CFPREV.FECHAEJECUCION, FORMAT(CAST('01/01/2000' AS DATETIME), 'dd/MM/yyyy')), 
          @COUNT_REGISTROS = COUNT(CFPREV.ID_CICLOFACTURACIONPDV)
        FROM WSXML_SFG.CICLOFACTURACIONPDV CFPREV
        WHERE CFPREV.SECUENCIA = (@cMAXSECUENCIAFACTURACION - 1) AND CFPREV.ACTIVE = 1
        GROUP BY ISNULL(CFPREV.FECHAEJECUCION, FORMAT(CAST('01/01/2000' AS DATETIME), 'dd/MM/yyyy'))
        ORDER BY COUNT(CFPREV.ID_CICLOFACTURACIONPDV)
    ...
    BEGIN
      RAISERROR('-20054 Hay mas de un ciclo de facturación con el mismo código de secuencia. Este es un problema de consistencia de datos.', 16, 1)
    END
...
CREATE PROCEDURE WSXML_SFG.SFGREPORTESTERMINAL_GetDepositSlipHeaderCount(
  @p_SECUENCIACICLO NUMERIC(22,0), 
  @p_ROWCOUNT_out NUMERIC(22,0) OUT
) AS
BEGIN
  ...
  DECLARE @cFECHAEJECUCION          DATETIME
	...
	EXEC WSXML_SFG.SFGREPORTESTERMINAL_CONSTANT 
    @l_NUMEROREGISTROSXPAGINA OUT
	  
  EXEC WSXML_SFG.SFGREPORTESTERMINAL_ExpectedCycleID 
    @p_SECUENCIACICLO, 
    @cCODCICLOFACTURACIONPDV OUT, 
    @cFECHAEJECUCION OUT
...
GO
CREATE PROCEDURE WSXML_SFG.SFGREPORTESTERMINAL_GetInvoiceReportItmLstForAgent(@p_SECUENCIACICLO NUMERIC(22,0), @p_PAGNUM NUMERIC(22,0)) AS
 BEGIN
	 SET NOCOUNT ON;
    DECLARE @cCODCICLOFACTURACIONPDV NUMERIC(22,0);
    DECLARE @cFECHAULTIMAEJECUCION   DATETIME;
	...
	BEGIN TRY
		EXEC WSXML_SFG.SFGREPORTESTERMINAL_ExpectedCycleID @p_SECUENCIACICLO, @cCODCICLOFACTURACIONPDV out, @cFECHAULTIMAEJECUCION OUT
		EXEC WSXML_SFG.SFGREPORTESTERMINAL_GetPagingParameters @p_PAGNUM, @xLOWERLIMIT OUT, @xUPPERLIMIT OUT
