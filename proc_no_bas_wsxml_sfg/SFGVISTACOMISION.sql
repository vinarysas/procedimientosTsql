...
CREATE FUNCTION WSXML_SFG.SFGVISTACOMISION_GetComisionProducto(@P_LINEADENEGOCIO NUMERIC(22,0), @p_CODPUNTODEVENTA NUMERIC(22,0), @p_CODPRODUCTO NUMERIC(22,0)) RETURNS nvarchar(2000) AS
  BEGIN RETURN 1; END
   /*DECLARE cCOMISIONVARIABLE  NUMBER;
       BEGIN
              SELECT COMISIONVARIABLE INTO cCOMISIONVARIABLE FROM PRODUCTO P
              WHERE ID_PRODUCTO=p_CODPRODUCTO AND ACTIVE=1;

               IF cCOMISIONVARIABLE > 0 THEN
    ...
    /*DECLARE
      cPLANTILLAASIGNADA NUMBER;
    BEGIN
      SELECT CODPLANTILLAPRODUCTO INTO cPLANTILLAASIGNADA FROM PUNTODEVENTAPLANTILLA PDVP
      WHERE CODPUNTODEVENTA = p_CODPUNTODEVENTA
        AND CODLINEADENEGOCIO = P_LINEADENEGOCIO
        AND ACTIVE = 1;
      IF cPLANTILLAASIGNADA > 0 THEN
        SELECT RC.ID_RANGOCOMISION, RC.NOMRANGOCOMISION, RC.CODTIPOCOMISION, RC.ANTICIPO, RCDT.VALOR INTO cCODRANGOCOMISION, cNOMRANGOCOMISION, cCODTIPOCOMISION, cCOMISIONANTICIPO, cVALCALCUCOMISION
          FROM PLANTILLAPRODUCTODETALLE PLDT, RANGOCOMISION RC, RANGOCOMISIONDETALLE RCDT
         WHERE PLDT.CODPLANTILLAPRODUCTO = cPLANTILLAASIGNADA
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      DECLARE
        cPLANTILLAVALIDA     NUMBER;
        cIDENTIFICADORPRODCT NUMBER;
      BEGIN
        BEGIN
          SELECT ID_PLANTILLAPRODUCTO INTO cPLANTILLAVALIDA FROM PLANTILLAPRODUCTO PLAN, PUNTODEVENTA PDV
           WHERE PDV.ID_PUNTODEVENTA            = p_CODPUNTODEVENTA
             AND PLAN.CODLINEADENEGOCIO         = P_LINEADENEGOCIO
             AND PLAN.CODCIUDAD                 = PDV.CODCIUDAD
             AND PLAN.CODAGRUPACIONPUNTODEVENTA = PDV.CODAGRUPACIONPUNTODEVENTA
             AND PLAN.CODREDPDV                 = PDV.CODREDPDV
             AND PLAN.ACTIVE                    = 1;
          IF cPLANTILLAVALIDA > 0 THEN
            SELECT ID_PLANTILLAPRODUCTODETALLE INTO cIDENTIFICADORPRODCT FROM PLANTILLAPRODUCTODETALLE WHERE CODPLANTILLAPRODUCTO = cPLANTILLAVALIDA AND CODPRODUCTO = p_CODPRODUCTO;
          ELSE
            RAISE exINVALIDASSIGN;
          END IF;
        EXCEPTION WHEN OTHERS THEN
          ...
          EXCEPTION WHEN OTHERS THEN
            ...
            EXCEPTION WHEN OTHERS THEN
              ...
              EXCEPTION WHEN OTHERS THEN
                ...
                BEGIN
                  SELECT ID_PLANTILLAPRODUCTO INTO cPLANTILLAVALIDA FROM PLANTILLAPRODUCTO PLAN
                  WHERE PLAN.CODLINEADENEGOCIO = P_LINEADENEGOCIO
                    AND PLAN.MASTERPLANTILLA = 1
                    AND PLAN.ACTIVE = 1;
                  IF cPLANTILLAVALIDA > 0 THEN
                    SELECT ID_PLANTILLAPRODUCTODETALLE INTO cIDENTIFICADORPRODCT FROM PLANTILLAPRODUCTODETALLE WHERE CODPLANTILLAPRODUCTO = cPLANTILLAVALIDA AND CODPRODUCTO = p_CODPRODUCTO;
                  ELSE
                  ...
                  END IF;
                EXCEPTION WHEN OTHERS THEN
                  SFGTMPTRACE.TraceLog('No se encontro comision unica para el punto de venta ' || PUNTODEVENTA_CODIGO_F(p_CODPUNTODEVENTA) || ', producto ' || PRODUCTO_NOMBRE_F(p_CODPRODUCTO) || ' (' || PRODUCTO_CODIGO_F(p_CODPRODUCTO) || '). CDC: ' );
                  RAISE exINVALIDASSIGN;
                END;
              END;
            END;
          END;
        END;
        IF cIDENTIFICADORPRODCT > 0 THEN
          SELECT RC.ID_RANGOCOMISION, RC.NOMRANGOCOMISION, RC.CODTIPOCOMISION, RC.ANTICIPO, RCDT.VALOR INTO cCODRANGOCOMISION, cNOMRANGOCOMISION, cCODTIPOCOMISION, cCOMISIONANTICIPO, cVALCALCUCOMISION
            FROM PLANTILLAPRODUCTODETALLE PLDT, RANGOCOMISION RC, RANGOCOMISIONDETALLE RCDT
           WHERE PLDT.ID_PLANTILLAPRODUCTODETALLE = cIDENTIFICADORPRODCT
             AND RC.ID_RANGOCOMISION = PLDT.CODRANGOCOMISION
             AND RC.ID_RANGOCOMISION = RCDT.CODRANGOCOMISION;
        ELSE
          RAISE exINVALIDASSIGN;
        END IF;
      END;
    END;
    --p_CODTIPOCOMISION_out  := cCODTIPOCOMISION;
    --RETURN cVALCALCUCOMISION;
    RETURN cNOMRANGOCOMISION*/
GO
