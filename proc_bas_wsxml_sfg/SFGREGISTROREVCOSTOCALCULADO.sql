...
CREATE     PROCEDURE WSXML_SFG.SFGREGISTROREVCOSTOCALCULADO_GetRecordValue(@p_CODREGISTROREVENUE NUMERIC(22,0),
  ...
  BEGIN
  SET NOCOUNT ON;
    SELECT @p_VALORCOSTO = VALORCOSTO FROM WSXML_SFG.REGISTROREVCOSTOCALCULADO
    WHERE CODREGISTROREVENUE = @p_CODREGISTROREVENUE AND CODCOSTOCALCULADO = @p_CODCOSTOCALCULADO;
  IF @@ROWCOUNT = 0
    SET @p_VALORCOSTO = 0;
  END;
GO
