...
CREATE     PROCEDURE WSXML_SFG.SFGINCENTIVOCOMISIONGLOBAL_SetIncentivoDependencia(@p_CODINCENTIVOCOMISIONGLOBAL NUMERIC(22,0),
 ...
 BEGIN
  ...
  SET NOCOUNT ON;
    ...
    END 
    BEGIN
		SELECT @existingdep = ID_INCENTIVOCOMISIONHISTORICO 
		FROM WSXML_SFG.INCENTIVOCOMISIONHISTORICO
		WHERE CODINCENTIVOCOMISIONGLOBAL = @p_CODINCENTIVOCOMISIONGLOBAL 
			AND FECHAINICIOVALIDEZ = CONVERT(DATETIME, CONVERT(DATE,@p_FECHAINICIOVALIDEZ));
		
		IF @@ROWCOUNT = 0 BEGIN
		  INSERT INTO WSXML_SFG.INCENTIVOCOMISIONHISTORICO (CODINCENTIVOCOMISIONGLOBAL, FECHAINICIOVALIDEZ, VALORFIJO, CODUSUARIOMODIFICACION)
		  VALUES (@p_CODINCENTIVOCOMISIONGLOBAL, CONVERT(DATETIME, CONVERT(DATE,@p_FECHAINICIOVALIDEZ)), @p_VALORFIJO, @p_CODUSUARIOMODIFICACION);
		END ELSE BEGIN
			UPDATE WSXML_SFG.INCENTIVOCOMISIONHISTORICO SET VALORFIJO = @p_VALORFIJO 
			WHERE ID_INCENTIVOCOMISIONHISTORICO = @existingdep;
		END
    END;
  END;
