...
CREATE     PROCEDURE WSXML_SFG.SFGINF_REPORTESESPECIALES_ReporteDinamicoCadenas(@p_CODLINEADENEGOCIO NUMERIC(22,0),
                                   @pg_CADENA           NVARCHAR(2000),
                                  @p_STRCODIGOEXCEP    NVARCHAR(2000)
                                   ) AS
GO
...
CREATE     PROCEDURE WSXML_SFG.SFGINF_REPORTESESPECIALES_ReporteDinamicoCadProducto(@p_CODPRODUCTO NUMERIC(22,0),
 ...
 BEGIN
    ...
    SET NOCOUNT ON;
      ...
      INNER JOIN PUNTODEVENTA PDV ON (PDV.ID_PUNTODEVENTA = FCT.ID_PUNTODEVENTA)
      WHERE FCT.ID_CICLOFACTURACIONPDV = WSXML_SFG.ULTIMO_CICLOFACTURACION(GETDATE())
      AND FCT.ID_PRODUCTO=@p_CODPRODUCTO
      AND FCT.CODIGOAGRUPACIONGTECH  =  CAST(@pg_CADENA AS NUMERIC(38,0))
...
SET @vLASTDATE='30/apr/2017';
    ...
    '    FROM VWKDAYLYREVENUEADJUSTMENTS PRF                                                                                 ' +
    '    INNER JOIN PRODUCTO     PRD ON (PRF.CODPRODUCTO     = PRD.ID_PRODUCTO)                                              ' +
    '    INNER JOIN TIPOPRODUCTO TPR ON (PRD.CODTIPOPRODUCTO = TPR.ID_TIPOPRODUCTO)                                          ' +
    '    WHERE PRF.FECHAARCHIVO BETWEEN TO_DATE(''' + ISNULL(@v_FECHA_I_C, '')  +''',''dd/MM/yyyy'') AND  TO_DATE(''' + ISNULL(@v_FECHA_F_C, '') + ''',''dd/MM/yyyy'') ' +
    '    AND TPR.CODLINEADENEGOCIO      = 1                                                                                  ' +
    --'    AND PRD.CODTIPOPRODUCTO        = 1     ' ||
...
CREATE     PROCEDURE WSXML_SFG.SFGINF_REPORTESESPECIALES_ReporteEcomVentas( @P_FECHAINICIO NVARCHAR(2000),
      ...
      SET NOCOUNT ON;
       ...
       SELECT            CAST( PDV.CODIGOGTECHPUNTODEVENTA AS NUMERIC(38,0))      AS       CODIGOPUNTOVENTA
          ...
          FROM               WSXML_SFG.PUNTODEVENTA PDV
          INNER JOIN         RUTAPDV         RPDV    ON  PDV.CODRUTAPDV = RPDV.ID_RUTAPDV
          INNER JOIN         FMR            FMAR     ON  RPDV.CODFMR = FMAR.ID_FMR
          INNER JOIN
          (
           ...
           INNER JOIN   PRDOCUTOAREAVENTAS  PR    ON    REG.CODPRODUCTO = PR.CODPRODUCTO
           INNER JOIN   TIPOPRODUCTOVENTAS TP     ON    PR.CODTIPOPRODUCTOVENTAS = TP.ID_TIPOPRODUCTOVENTAS
           INNER JOIN   ENTRADAARCHIVOCONTROL E ON E.ID_ENTRADAARCHIVOCONTROL = REG.CODENTRADAARCHIVOCONTROL
          ...
          INNER JOIN      TIPOPRODUCTOVENTAS   TPV     ON    RGF.CODIGOVENTAS  = TPV.codigoventas
          WHERE           RGF.VENTAS <>  0
