...
CREATE     PROCEDURE WSXML_SFG.SFGREGFACTURACIONREFERENCIA_FindReference(@p_CODENTRADAARCHIVOCONTROL NUMERIC(22,0),
  ...
  SET NOCOUNT ON;
    ...
    BEGIN
      ...
      BEGIN
        ...
        SET @p_ID_REGFACTREFERENCIA_out = SCOPE_IDENTITY();
			...
			IF @@ROWCOUNT = 0 BEGIN
				...
				SELECT @cCODHUERFANOSC = ID_HUERFANOSERVICIOSCOMERCIALS FROM WSXML_SFG.HUERFANOSERVICIOSCOMERCIALES
				WHERE CODENTRADAARCHIVOCONTROL = @p_CODENTRADAARCHIVOCONTROL
	  ...
	  IF @@ROWCOUNT = 0 BEGIN
		set @msg = '-20010 No se encontro la venta a la que se hace referencia: Archivo ID ' + ISNULL(@p_CODENTRADAARCHIVOCONTROL, '') + ', POS ' + ISNULL(@p_PUNTOVENTA, '') + ', Producto ' + ISNULL(@p_CODIGOPRODUCTO, '') + ', Referencia ' + ISNULL(@p_NUMEROREFERENCIA, '')
		RAISERROR(@msg, 16, 1);
	  END

		IF @@ROWCOUNT > 1 BEGIN
		set @msg = '-20011 No se pueden cargar referencias a archivos no agrupados: Archivo ID ' + ISNULL(@p_CODENTRADAARCHIVOCONTROL, '') + ', POS ' + ISNULL(@p_PUNTOVENTA, '') + ', Producto ' + ISNULL(@p_CODIGOPRODUCTO, '') + ', Referencia ' + ISNULL(@p_NUMEROREFERENCIA, '')
		RAISERROR(@msg, 16, 1);
	  END
