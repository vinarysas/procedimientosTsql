...
CREATE     PROCEDURE WSXML_SFG.SFGSINCRONIZACIONSAG_InsertUpdateJefeDistrito(@p_CODUSUARIO              NUMERIC(22,0),
                                     @p_ACTIVE                  NUMERIC(22,0),
 ...
 BEGIN
  ...
  SET NOCOUNT ON;
    ...
    SELECT @vCOUNT = COUNT(1)
     ...
     WHERE JEFEDISTRITO.CODIGOGTECHJEFEDISTRITO = @p_CODIGOGTECHJEFEDISTRITO;
  
    IF @vCOUNT = 0 BEGIN
      --it does not exist and must to be created 
      EXEC WSXML_SFG.SFGJEFEDISTRITO_AddJefeRecord  @p_CODIGOGTECHJEFEDISTRITO,
                                    @p_NOMJEFEDISTRITO,
                                    @vID_USUARIOSISTEMA,
                                    @vID_JEFEDISTRITO OUT
     END 
      EXEC WSXML_SFG.SFGJEFEDISTRITO_CheckUpdateRecord @p_CODIGOGTECHJEFEDISTRITO,
                                        @p_NOMJEFEDISTRITO,
                                        @p_EMAIL
   
  
    SELECT @vCOUNT = COUNT(1) FROM WSXML_SFG.USUARIO WHERE USUARIO.EMAIL = @p_EMAIL;
...
CREATE     PROCEDURE WSXML_SFG.SFGSINCRONIZACIONSAG_InsertUpdateRegional(@p_NOMREGIONAL     NVARCHAR(2000),
 ...
 BEGIN
  ...
  SET NOCOUNT ON;
    SELECT @vCOUNT = COUNT(1)
      FROM WSXML_SFG.JEFEDISTRITO
     WHERE JEFEDISTRITO.CODIGOGTECHJEFEDISTRITO = @p_CODJEFEDISTRITO;
...
CREATE     PROCEDURE WSXML_SFG.SFGSINCRONIZACIONSAG_InsertUpdatePDVRedExterna(@p_CODDUENOPUNTODEVENTA      NUMERIC(22,0),
    ...
    BEGIN
      SELECT    @vID_AGRUPACIONPDV = CODAGRUPACIONPUNTODEVENTA,@vID_RAZONSOCIAL = CODRAZONSOCIAL 
		...
		IF @@ROWCOUNT = 0 BEGIN
			SET @MSGERROR='No se encontro un due?o de puntos de venta con el codigo ' + ISNULL(CONVERT(VARCHAR, @p_CODDUENOPUNTODEVENTA), '');
			RAISERROR(-20000, @MSGERROR, 16, 1);
		END
    END;  
    
    IF @vID_AGRUPACIONPDV IS NULL BEGIN 
        SET @MSGERROR='No se encontro una cadena configurada en el dueno de punto de venta codigo :' + ISNULL(CONVERT(VARCHAR, @p_CODDUENOPUNTODEVENTA), '');
        RAISERROR(-20000, @MSGERROR, 16, 1);
    END 
    ...
    IF LEN(RTRIM(LTRIM(@p_NOMREGIONAL)))<>0
      BEGIN 
          ...
          WHERE NOMREGIONAL = @p_NOMREGIONAL;
			...
			IF @@ROWCOUNT = 0 BEGIN
              SET @MSGERROR = ' No se puede crear el punto de venta   ' + ISNULL(@p_CODIGOPUNTODEVENTA, '') + ' por que no existe una regional con el nombre : ' + ISNULL(@p_NOMREGIONAL, '');
              RAISERROR(-20000, @MSGERROR, 16, 1);
			END
      END;
    ELSE 
