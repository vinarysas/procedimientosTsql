...
CREATE     PROCEDURE WSXML_SFG.SFGINF_CRUZADOLINEAS_GetMonthlyHeaders(@p_CODCICLOFACTURACIONPDV NUMERIC(22,0)) AS
 BEGIN
  ...
  SET NOCOUNT ON;
    ...
    begin
		...
		BEGIN
			...
			UPDATE @lstCICLOSFACTURACION SET FECHAINICIO = @iniDATE, FECHAFIN = @endDATE WHERE ID_CICLOFACTURACIONPDV = @ix__ID_CICLOFACTURACIONPDV
    ...
    IF @xFECHAFRST < @sFECHAFRST AND @xFECHALAST < @sFECHALAST BEGIN
        ...
        SELECT SECUENCIA AS ORDEN,
               '(' + ISNULL(CONVERT(VARCHAR,WSXML_SFG.SFG_PACKAGE_GetYearNumber(FECHAFIN)), '') + '-' + ISNULL(CONVERT(VARCHAR,WSXML_SFG.SFG_PACKAGE_GetWeekNumber(FECHAFIN)), '') + ') ' +
