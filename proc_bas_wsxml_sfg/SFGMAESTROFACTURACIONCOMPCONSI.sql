...
CREATE FUNCTION WSXML_SFG.SFGMAESTROFACTURACIONCOMPCONSI_GroupedReferenceNumber(@p_CODIGOAGRUPACIONGTECH NVARCHAR(2000), @p_FECHACIERRE DATETIME) RETURNS VARCHAR(4000) AS
 BEGIN
    --
    ...
    --
     ...
     --
     --RETURN ReferenceNumber(xGROUPED, p_CODIGOAGRUPACIONGTECH, NULL, p_FECHACIERRE);
     --
     SELECT @xAgrupacionPuntoVenta = ID_AGRUPACIONPUNTODEVENTA
       FROM WSXML_SFG.agrupacionpuntodeventa
      WHERE codigoagrupaciongtech = @p_CODIGOAGRUPACIONGTECH;
     --
     EXEC WSXML_SFG.SFGMAESTROFACTURACIONCOMPCONSI_GetNumeroReferencia @xGROUPED, @xAgrupacionPuntoVenta, NULL, @p_FECHACIERRE, @xREFERENCIA OUT
     --
     RETURN @xREFERENCIA;
     --
  END;
