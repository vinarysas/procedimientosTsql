...
CREATE     PROCEDURE WSXML_SFG.SFGINF_REPORTESESPECIALES_CutoffContable (@P_CODCICLOFACTURACIONPDV NUMERIC(22,0),
  ...
  AS
  BEGIN
    DECLARE @ALIADOS       WSXML_SFG.IDSTRINGVALUE;
    DECLARE @ALIADOSHEADER NVARCHAR(MAX);
    DECLARE @ALIADONAME VARCHAR(30);
    DECLARE @v_FECHA_I DATETIME;
    DECLARE @v_FECHA_F DATETIME;
    DECLARE @v_FECHA_I_C VARCHAR(30);
    DECLARE @v_FECHA_F_C VARCHAR(30);
    DECLARE @v_FECHACCLO DATETIME;

 
SET NOCOUNT ON;

      IF @P_CODCICLOFACTURACIONPDV =  '-1' BEGIN
                   SELECT @v_FECHACCLO = FECHAEJECUCION
                   FROM WSXML_SFG.CICLOFACTURACIONPDV
                   WHERE ID_CICLOFACTURACIONPDV IN
                         (
                           SELECT MAX (CPV.ID_CICLOFACTURACIONPDV) FROM  WSXML_SFG.CICLOFACTURACIONPDV CPV
                         );
     ...
     EXEC WSXML_SFG.SFG_PACKAGE_GetMonthRange @v_FECHACCLO, @v_FECHA_I OUT , @v_FECHA_F OUT

     SET @v_FECHA_I_C = FORMAT( @v_FECHA_I,'dd/MM/yyyy');
     SET @v_FECHA_F_C = FORMAT( @v_FECHA_F,'dd/MM/yyyy');


	 INSERT INTO   @ALIADOS
     SELECT ID_ALIADOESTRATEGICO,NOMALIADOESTRATEGICO
     ...
     --  WHERE ID_ALIADOESTRATEGICO <= 109
     ORDER  BY NOMALIADOESTRATEGICO;

     IF @@ROWCOUNT > 0 BEGIN
			DECLARE iax CURSOR FOR SELECT ID, VALUE FROM @aliados
			OPEN iax

			DECLARE @iax__ID NUMERIC, @iax__VALUE NVARCHAR(2000)
			FETCH NEXT FROM iax INTO @iax__ID, @iax__VALUE 

			WHILE (@@FETCH_STATUS = 0) BEGIN

				IF @iax__ID = 428
						 SET @ALIADONAME= 'EMP-PUB-MED-REG(BP)';
				ELSE IF @iax__ID = 266 
						 SET @ALIADONAME= 'EMP-PUB-MED-(BP)';
				IF @iax__ID = 110 
						 SET @ALIADONAME= 'EMP-PUB-MED-REG(PINES)';
				ELSE
					SET @ALIADONAME = SUBSTRING(@iax__VALUE, 0,24);
                  

                      SET @aliadosheader = isnull(@ALIADOSHEADER, '') +
                       ',CONVERT(NUMERIC,SUM(CASE WHEN ALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(CANTDTX,0) ELSE 0 END)) AS "'+ ISNULL(@ALIADONAME, '') +' - Tx"' +
                       ',CAST(ROUND(SUM(CASE WHEN ALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(VALUETX,0) ELSE 0 END),2,1) AS DECIMAL(16,2)) AS "'+ ISNULL(@ALIADONAME, '') +' - Vlr"' +
                       ',CAST(ROUND(SUM(CASE WHEN ALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(RVNUETX,0) ELSE 0 END),2,1) AS DECIMAL(16,2)) AS "'+ ISNULL(@ALIADONAME, '') +' - Rvn"';
                FETCH NEXT FROM iax INTO @iax__ID, @iax__VALUE 
			END
			CLOSE iax;
			DEALLOCATE iax;
     END
      ...
      DECLARE @sql NVARCHAR(MAX) 
		SET @sql = ' SELECT SFG_CONCILIACION.CON_CALENDARIO_GRAL.FECHA_CALENDARIO AS "Fecha" ' + isnull(@ALIADOSHEADER, '') +
          ' FROM SFG_CONCILIACION.CON_CALENDARIO_GRAL  ' +
          ' INNER JOIN wsxml_sfg.CONFIGURACIONALIADOARCHIVO CAA ON (1 = 1) ' +
          ' LEFT OUTER JOIN (SELECT PRD.CODALIADOESTRATEGICO AS ALIADO, CTR.FECHAARCHIVO, ' +
          '                        SUM(CASE WHEN REG.CODTIPOREGISTRO = 1 THEN ISNULL(REG.NUMTRANSACCIONES,0) WHEN REG.CODTIPOREGISTRO = 2 THEN ISNULL(REG.NUMTRANSACCIONES,0) * (-1) ELSE 0 END) AS CANTDTX, ' +
          '                        SUM(CASE WHEN REG.CODTIPOREGISTRO = 1 THEN ISNULL(REG.VALORTRANSACCION,0) WHEN REG.CODTIPOREGISTRO = 2 THEN ISNULL(REG.VALORTRANSACCION,0) * (-1) ELSE 0 END) AS VALUETX, ' +
          '                        SUM(CASE WHEN REG.CODTIPOREGISTRO = 1 THEN ISNULL(REV.REVENUETOTAL,0)     WHEN REG.CODTIPOREGISTRO = 2 THEN ISNULL(REV.REVENUETOTAL,0)     * (-1) ELSE 0 END) AS RVNUETX  ' +
          '                 FROM wsxml_sfg.ENTRADAARCHIVOCONTROL CTR ' +
          '                 INNER JOIN wsxml_sfg.REGISTROFACTURACION REG ON (REG.CODENTRADAARCHIVOCONTROL = ID_ENTRADAARCHIVOCONTROL) ' +
          '                 INNER JOIN wsxml_sfg.REGISTROREVENUE REV ON (REV.CODREGISTROFACTURACION = ID_REGISTROFACTURACION) ' +
          '                 INNER JOIN wsxml_sfg.PRODUCTO PRD ON (PRD.ID_PRODUCTO = REG.CODPRODUCTO) ' +
          '                 WHERE CTR.REVERSADO = 0 AND CTR.TIPOARCHIVO = 1 AND CTR.FECHAARCHIVO BETWEEN CONVERT(DATETIME,''' + ISNULL(@v_FECHA_I_C, '') +''', 103) AND CONVERT(DATETIME,''' + ISNULL(@v_FECHA_F_C, '') +''', 103) ' +
          '                 GROUP BY CTR.FECHAARCHIVO, PRD.CODALIADOESTRATEGICO) ARC ON (ARC.FECHAARCHIVO = SFG_CONCILIACION.CON_CALENDARIO_GRAL.FECHA_CALENDARIO AND ARC.ALIADO = CAA.CODALIADOESTRATEGICO) ' +
          ' WHERE SFG_CONCILIACION.CON_CALENDARIO_GRAL.FECHA_CALENDARIO BETWEEN CONVERT(DATETIME,''' + ISNULL(@v_FECHA_I_C, '') +''', 103) AND CONVERT(DATETIME,''' + ISNULL(@v_FECHA_F_C, '') +''', 103) ' +
          ' GROUP BY SFG_CONCILIACION.CON_CALENDARIO_GRAL.FECHA_CALENDARIO ' +
          ' ORDER BY SFG_CONCILIACION.CON_CALENDARIO_GRAL.FECHA_CALENDARIO DESC';
...
CREATE     PROCEDURE WSXML_SFG.SFGINF_REPORTESESPECIALES_CutoffPlanos                (@p_CODCICLOFACTURACIONPDV NUMERIC(22,0),
  ...
  BEGIN
     DECLARE @ALIADOS       WSXML_SFG.IDSTRINGVALUE;
     DECLARE @ALIADOSHEADER NVARCHAR(MAX);
     DECLARE @ALIADONAME NVARCHAR(30);
     DECLARE @v_FECHA_I DATETIME;
     DECLARE @v_FECHA_F DATETIME;
     DECLARE @v_FECHA_I_C VARCHAR(30);
     DECLARE @v_FECHA_F_C VARCHAR(30);
  ...
  IF @@ROWCOUNT > 0 BEGIN
	...
	WHILE (@@FETCH_STATUS = 0) BEGIN
     ...
     SET @ALIADOSHEADER = ISNULL(@ALIADOSHEADER,'') +
                       CAST(',CAST(ROUND(SUM(CASE WHEN ALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(CANTDTX,0) ELSE 0 END),0,1) AS DECIMAL(16,0)) AS "'+ ISNULL(@ALIADONAME, '') +' - Tx"' AS NVARCHAR(MAX)) +
                       CAST(',CAST(ROUND(SUM(CASE WHEN ALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(VALUETX,0) ELSE 0 END),2,1) AS DECIMAL(16,2)) AS "'+ ISNULL(@ALIADONAME, '') +' - Vlr"' AS NVARCHAR(MAX)) +
                       CAST(',CAST(ROUND(SUM(CASE WHEN ALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(RVNUETX,0) ELSE 0 END),2,1) AS DECIMAL(16,2)) AS "'+ ISNULL(@ALIADONAME, '') +' - Rvn"' AS NVARCHAR(MAX))
     FETCH NEXT FROM iax INTO @iax__ID, @iax__VALUE 
    END
    CLOSE iax;
    DEALLOCATE iax;
  ...
  --PRINT LEN(@ALIADOSHEADER)
  --PRINT CAST(@ALIADOSHEADER AS NVARCHAR(MAX))

  DECLARE @sql NVARCHAR(MAX) 
    SET @sql = ' SELECT CAL.FECHA_CALENDARIO AS "Fecha" ' + CAST(@ALIADOSHEADER AS VARCHAR(MAX)) +
    ' FROM SFG_CONCILIACION.CON_CALENDARIO_GRAL CAL ' +
    ' LEFT OUTER JOIN (SELECT ATA.CODALIADOESTRATEGICO AS ALIADO, ATA.FECHAARCHIVO, ' +
    '                        COUNT(TXA.ID_TRANSACCIONALIADO)        AS CANTDTX, ' +
    '                        SUM(ISNULL(TXA.VALORTRANSACCION,0))       AS VALUETX, ' +
    '                        SUM(ISNULL(TXA.REVENUETRANSACCION,0))     AS RVNUETX  ' +
    '                 FROM wsxml_sfg.ARCHIVOTRANSACCIONALIADO ATA ' +
    '                 INNER JOIN wsxml_sfg.TRANSACCIONALIADO TXA ON (TXA.CODARCHIVOTRANSACCIONALIADO = ATA.ID_ARCHIVOTRANSACCIONALIADO) ' +
    '                 WHERE FECHAARCHIVO BETWEEN CONVERT(DATETIME,''' + ISNULL(@v_FECHA_I_C, '') +''', 103) AND CONVERT(DATETIME,''' + ISNULL(@v_FECHA_F_C, '') +''', 103) ' +
    '                 GROUP BY ATA.CODALIADOESTRATEGICO, ATA.FECHAARCHIVO) ARC ON (ARC.FECHAARCHIVO = CAL.FECHA_CALENDARIO) ' +
    ' WHERE CAL.FECHA_CALENDARIO BETWEEN CONVERT(DATETIME,''' + ISNULL(@v_FECHA_I_C, '') +''',103) AND CONVERT(DATETIME,''' + ISNULL(@v_FECHA_F_C, '') +''', 103) ' +
    ' GROUP BY CAL.FECHA_CALENDARIO ' +
    ' ORDER BY CAL.FECHA_CALENDARIO DESC';
	EXECUTE sp_executesql @sql;
  END;
GO
...
CREATE PROCEDURE WSXML_SFG.SFGINF_REPORTESESPECIALES_CutoffDiferencia (
...
BEGIN
  ...
  BEGIN
    ...
    FROM WSXML_SFG.CICLOFACTURACIONPDV
    WHERE ID_CICLOFACTURACIONPDV IN (
      SELECT MAX(CPV.ID_CICLOFACTURACIONPDV) 
      FROM  WSXML_SFG.CICLOFACTURACIONPDV CPV
    )
  END    
  ELSE 
  BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      END
  		...
  		SET @ALIADOSHEADER = 
        ISNULL(@ALIADOSHEADER, '') +
        CAST(',CONVERT(NUMERIC,(SUM(CASE WHEN CNTALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(CNTCANTDTX,0) ELSE 0 END)) - CONVERT(NUMERIC,SUM(CASE WHEN ALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(CANTDTX,0) ELSE 0 END))) AS "'+ ISNULL(@ALIADONAME, '') +' - Tx"'  AS NVARCHAR(MAX)) +
        CAST(',CAST(ROUND(SUM(CASE WHEN CNTALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(CNTVALUETX,0) ELSE 0 END),2,1) AS DECIMAL(16,2)) - CAST(ROUND(SUM(CASE WHEN ALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(VALUETX,0) ELSE 0 END),2,1) AS DECIMAL(16,2)) AS "'+ ISNULL(@ALIADONAME, '') +' - Vlr"' AS NVARCHAR(MAX)) +
        CAST(',CAST(ROUND(SUM(CASE WHEN CNTALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(CNTRVNUETX,0) ELSE 0 END),2,1) AS DECIMAL(16,2)) - CAST(ROUND(SUM(CASE WHEN ALIADO=' + CONVERT(VARCHAR,@iax__ID) + ' THEN ISNULL(RVNUETX,0) ELSE 0 END),2,1) AS DECIMAL(16,2)) AS "'+ ISNULL(@ALIADONAME, '') +' - Rvn"' AS NVARCHAR(MAX));
  ...
  --SELECT CAST(@ALIADOSHEADER AS VARCHAR(MAX))
  DECLARE @sql NVARCHAR(MAX);
    SET @sql = 
      'SELECT CNTCALENDARIO.FECHA_CALENDARIO AS "Fecha" ' + CAST(ISNULL(@ALIADOSHEADER,'') AS NVARCHAR(MAX)) +
      ' FROM SFG_CONCILIACION.CON_CALENDARIO_GRAL  CNTCALENDARIO ' +
      ' INNER JOIN wsxml_sfg.CONFIGURACIONALIADOARCHIVO CNTCAA ON (1 = 1) ' +
      ' LEFT OUTER JOIN (SELECT CNTPRD.CODALIADOESTRATEGICO AS CNTALIADO, CNTENT.FECHAARCHIVO, ' +
      ' SUM(CASE WHEN CNTREG.CODTIPOREGISTRO = 1 THEN ISNULL(CNTREG.NUMTRANSACCIONES,0) WHEN CNTREG.CODTIPOREGISTRO = 2 THEN ISNULL(CNTREG.NUMTRANSACCIONES,0) * (-1) ELSE 0 END) AS CNTCANTDTX, ' +
      ' SUM(CASE WHEN CNTREG.CODTIPOREGISTRO = 1 THEN ISNULL(CNTREG.VALORTRANSACCION,0) WHEN CNTREG.CODTIPOREGISTRO = 2 THEN ISNULL(CNTREG.VALORTRANSACCION,0) * (-1) ELSE 0 END) AS CNTVALUETX, ' +
      ' SUM(CASE WHEN CNTREG.CODTIPOREGISTRO = 1 THEN ISNULL(CNTREV.REVENUETOTAL,0) WHEN CNTREG.CODTIPOREGISTRO = 2 THEN ISNULL(CNTREV.REVENUETOTAL,0) * (-1) ELSE 0 END) AS CNTRVNUETX  ' +
      ' FROM wsxml_sfg.ENTRADAARCHIVOCONTROL CNTENT ' +
      ' INNER JOIN wsxml_sfg.REGISTROFACTURACION CNTREG ON (CNTREG.CODENTRADAARCHIVOCONTROL = CNTENT.ID_ENTRADAARCHIVOCONTROL) ' +
      ' INNER JOIN wsxml_sfg.REGISTROREVENUE CNTREV ON (CNTREV.CODREGISTROFACTURACION = CNTREG.ID_REGISTROFACTURACION) ' +
      ' INNER JOIN wsxml_sfg.PRODUCTO CNTPRD ON (CNTPRD.ID_PRODUCTO = CNTREG.CODPRODUCTO) ' +
      ' WHERE CNTENT.REVERSADO = 0 AND CNTENT.TIPOARCHIVO = 1 AND CNTENT.FECHAARCHIVO BETWEEN CONVERT(DATETIME,''' + ISNULL(@v_FECHA_I_C, '') +''', 103) AND CONVERT(DATETIME,''' + ISNULL(@v_FECHA_F_C, '') +''', 103) ' +
      ' GROUP BY CNTENT.FECHAARCHIVO, CNTPRD.CODALIADOESTRATEGICO) CNTARC ON (CNTARC.FECHAARCHIVO = CNTCALENDARIO.FECHA_CALENDARIO AND CNTARC.CNTALIADO = CNTCAA.CODALIADOESTRATEGICO) ' +
      ' LEFT OUTER JOIN (SELECT ATA.CODALIADOESTRATEGICO AS ALIADO, ATA.FECHAARCHIVO, ' +
      ' COUNT(TXA.ID_TRANSACCIONALIADO) AS CANTDTX, ' +
      ' SUM(ISNULL(TXA.VALORTRANSACCION,0)) AS VALUETX, ' +
      ' SUM(ISNULL(TXA.REVENUETRANSACCION,0)) AS RVNUETX  ' +
      ...
      ' INNER JOIN wsxml_sfg.TRANSACCIONALIADO TXA ON (TXA.CODARCHIVOTRANSACCIONALIADO = ATA.ID_ARCHIVOTRANSACCIONALIADO) ' +
      ' WHERE FECHAARCHIVO BETWEEN CONVERT(DATETIME,''' + ISNULL(@v_FECHA_I_C, '') +''', 103) AND CONVERT(DATETIME,''' + ISNULL(@v_FECHA_F_C, '') +''', 103) ' +
      ' GROUP BY ATA.CODALIADOESTRATEGICO, ATA.FECHAARCHIVO ' +
      ' --ORDER BY  ATA.FECHAARCHIVO 
		  ) ARC ON (ARC.FECHAARCHIVO = CNTCALENDARIO.FECHA_CALENDARIO) AND (ARC.ALIADO = CNTALIADO) ' +
      ' WHERE CNTCALENDARIO.FECHA_CALENDARIO BETWEEN CONVERT(DATETIME,''' + ISNULL(@v_FECHA_I_C, '') +''', 103) AND CONVERT(DATETIME,''' + ISNULL(@v_FECHA_F_C, '') +''', 103) ' +
      ' GROUP BY CNTCALENDARIO.FECHA_CALENDARIO ' +
      ' ORDER BY CNTCALENDARIO.FECHA_CALENDARIO DESC';
