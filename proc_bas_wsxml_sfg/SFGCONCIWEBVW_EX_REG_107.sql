...
--    1) count is 0 ;
--    2) empty resultset.
IF OBJECT_ID('WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_GetList', 'P') IS NOT NULL
  DROP PROCEDURE WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_GetList;
GO

CREATE PROCEDURE WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_GetList(
...
SET NOCOUNT ON;
    ...
    SET @l_from_str = 'SFG_CONCILIACION.VW_EX_REG_107';
    SET @l_alias_str = 'VW_EX_REG_107';
    ...
    BEGIN
        SET @l_count_query =
            'SELECT /*+ NO_MERGE(VW_EX_REG_107) */ count(*) ' +
    ...
    SET @l_query_cols =
        'VW_EX_REG_107.ALIADO,
            VW_EX_REG_107.FECHA_ARCHIVO,
            VW_EX_REG_107.FEC_POSTEO,
            VW_EX_REG_107.TIPO_TRX,
            VW_EX_REG_107.FEC_HORA_TRX,
            VW_EX_REG_107.NRO_CUENTA,
            VW_EX_REG_107.NRO_TARJETA,
            VW_EX_REG_107.VALOR,
            VW_EX_REG_107.CITI_ERROR,
            VW_EX_REG_107.CONF_TRX,
            VW_EX_REG_107.DESCIP_TERM,
            VW_EX_REG_107.REVERSAL,
            VW_EX_REG_107.VALOR_REVERSADO,
            VW_EX_REG_107.SPRN,
            VW_EX_REG_107.ESTADO_GTECH,
            VW_EX_REG_107.MOTIVO_DIFERENCIA,
            VW_EX_REG_107.CAMPOS_FALLO';
    ...
    BEGIN
        ...
        THEN
            l_sort_str := 'ORDER BY ' || p_sort_str;
        ELSE
            l_sort_str := ' ';
        END IF;*/
...
-- given the search criteria and sorting condition.
IF OBJECT_ID('WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_DrillDown', 'P') IS NOT NULL
  DROP PROCEDURE WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_DrillDown;
GO

CREATE PROCEDURE WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_DrillDown(
...
SET NOCOUNT ON;
    ...
    BEGIN
        ...
        BEGIN
            SET @l_count_query =
                'SELECT /*+ NO_MERGE(VW_EX_REG_107) */ count(*) FROM ( SELECT ' + isnull(@p_select_str, '') +
                'FROM ' + isnull(@l_from_str, '') + ' ' + isnull(@l_join_str + ' ' +
                isnull(@l_where_str, '') + ' ) countAlias', '');
        END
        ELSE BEGIN
            SET @l_count_query =
                'SELECT /*+ NO_MERGE(VW_EX_REG_107) */ COUNT(*) FROM ( SELECT DISTINCT ' + isnull(@p_select_str, '') + ', 1 As count1  ' +
                'FROM ' + isnull(@l_from_str, '') + ' ' + isnull(@l_join_str, '') + ' ' +
...
-- Returns a query result from table VW_EX_REG_107
-- given the search criteria and sorting condition.
IF OBJECT_ID('WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_GetStats', 'P') IS NOT NULL
  DROP PROCEDURE WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_GetStats;
GO

CREATE PROCEDURE WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_GetStats(
    @p_select_str NVARCHAR(2000),
    @p_join_str NVARCHAR(2000),
    @p_where_str NVARCHAR(2000),
    @p_sort_str NVARCHAR(2000),
...
IF OBJECT_ID('WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_Export', 'P') IS NOT NULL
  DROP PROCEDURE WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_Export;
GO

CREATE PROCEDURE WSXML_SFG.SFGCONCIWEBVW_EX_REG_107_Export(
...
SET NOCOUNT ON;
    ...
    SET @l_from_str = 'SFG_CONCILIACION.VW_EX_REG_107 VW_EX_REG_107_ LEFT OUTER JOIN WSXML_SFG.VW_ALIADOESTRATEGICO t0 ON (VW_EX_REG_107_.ALIADO =  t0.ID_ALIADOESTRATEGICO)';
