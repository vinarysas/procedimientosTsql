...
IF EXISTS (
    SELECT * FROM sys.objects WHERE OBJECT_NAME(object_id) = N'SFGPRODUCTOCONTRATO_GetTarifaCacheList_F'
    AND type IN (N'FN', N'IF', N'TF')
)
DROP FUNCTION WSXML_SFG.SFGPRODUCTOCONTRATO_GetTarifaCacheList_F
GO

CREATE     FUNCTION WSXML_SFG.SFGPRODUCTOCONTRATO_GetTarifaCacheList_F(@p_FECHA DATETIME,@p_CODPRODUCTO NUMERIC(22,0) = -1)
 RETURNS @getTarifaCacheList TABLE(PARENT NUMERIC(22,0), CODTARIFAVALOR NUMERIC(22,0), VALOR FLOAT)
    AS
  BEGIN
...
IF EXISTS (
    SELECT * FROM sys.objects WHERE OBJECT_NAME(object_id) = N'SFGPRODUCTOCONTRATO_GetTarifaCacheListF'
    AND type IN (N'FN', N'IF', N'TF')
)
     DROP FUNCTION WSXML_SFG.SFGPRODUCTOCONTRATO_GetTarifaCacheListF;
GO

CREATE     FUNCTION WSXML_SFG.SFGPRODUCTOCONTRATO_GetTarifaCacheListF(@p_FECHA DATETIME,@p_CODPRODUCTO NUMERIC(22,0) = -1) 
RETURNS @tarifacachelist TABLE  (
	PARENT numeric(38,0),
	CODTARIFAVALOR NUMERIC(22,0),
	VALOR FLOAT
) AS
 BEGIN
...
CREATE     FUNCTION WSXML_SFG.SFGPRODUCTOCONTRATO_TranslateTarifaFromMaster(--p_CACHETARIFA               PRODUCTTARIFALIST,
                                     ...
                                     @p_CODTARIFAVALOR            NUMERIC(22,0),
									 @getTarifaCacheList    	  WSXML_SFG.PRODUCTTARIFA READONLY
									 ) RETURNS FLOAT AS
 BEGIN
    ...
    IF @p_CODPRODUCTOCONTRATOCOMDIF IS NOT NULL
      BEGIN
        ...
        WHERE PARENT = @p_CODPRODUCTOCONTRATOCOMDIF AND CODTARIFAVALOR = @p_CODTARIFAVALOR;
			...
			BEGIN
			   SELECT @p_VALOR_out = VALOR FROM @getTarifaCacheList
