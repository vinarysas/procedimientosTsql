USE SFGPRODU;
--  DDL for Function COMISIONSIMPLE_F
--------------------------------------------------------

-- Se migro de funcion a procedimiento debido a que no soporta update internos las funciones
-- De igual forma se cambio de donde esta era llamada:
-- SFGPLANTILLAPRODUCTO.sql
-- SFGPRODUCTOCONTRATO.sql

  IF OBJECT_ID('WSXML_SFG.COMISIONSIMPLE_F', 'P') IS NOT NULL
    DROP PROCEDURE WSXML_SFG.COMISIONSIMPLE_F;
GO


  CREATE PROCEDURE WSXML_SFG.COMISIONSIMPLE_F (@p_CODTIPOCOMISION NUMERIC(22,0), @p_VALORPORCENTUAL NUMERIC(22,0), @p_VALORTRANSACCIONAL NUMERIC(22,0), @commissionid NUMERIC(22,0) OUT) AS
 BEGIN
  ...
  DECLARE @l_COMISIONFORMATSIMPLE_F VARCHAR(2000)
 
  IF @p_CODTIPOCOMISION = 1 -- Porcentual
    BEGIN
      SELECT @commissionid = ID_RANGOCOMISION FROM WSXML_SFG.RANGOCOMISION
      INNER JOIN WSXML_SFG.RANGOCOMISIONDETALLE ON (CODRANGOCOMISION = ID_RANGOCOMISION)
      WHERE CODTIPOCOMISION = @p_CODTIPOCOMISION AND VALORPORCENTUAL = @p_VALORPORCENTUAL AND VALORTRANSACCIONAL = 0 AND CODINCENTIVOCOMISIONGLOBAL IS NULL;
  ...
  ELSE IF @p_CODTIPOCOMISION = 2 -- Transaccional
    BEGIN
      SELECT @commissionid = ID_RANGOCOMISION FROM WSXML_SFG.RANGOCOMISION
      INNER JOIN WSXML_SFG.RANGOCOMISIONDETALLE ON (CODRANGOCOMISION = ID_RANGOCOMISION)
      WHERE CODTIPOCOMISION = @p_CODTIPOCOMISION AND VALORPORCENTUAL = 0 AND VALORTRANSACCIONAL = @p_VALORTRANSACCIONAL AND CODINCENTIVOCOMISIONGLOBAL IS NULL;
		IF @@ROWCOUNT = 0 BEGIN
  ...
  ELSE IF @p_CODTIPOCOMISION = 3 -- Mixto
    BEGIN
      SELECT @commissionid = ID_RANGOCOMISION FROM WSXML_SFG.RANGOCOMISION
      INNER JOIN WSXML_SFG.RANGOCOMISIONDETALLE ON (CODRANGOCOMISION = ID_RANGOCOMISION)
      WHERE CODTIPOCOMISION = @p_CODTIPOCOMISION AND VALORPORCENTUAL = @p_VALORPORCENTUAL AND VALORTRANSACCIONAL = @p_VALORTRANSACCIONAL AND CODINCENTIVOCOMISIONGLOBAL IS NULL;
		IF @@ROWCOUNT = 0 BEGIN
    ...
    END;
  ELSE
    RAISERROR('-20054 No se puede obtener una comision compuesta a partir de la funcion',16,1);
