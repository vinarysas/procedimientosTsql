...
CREATE     PROCEDURE WSXML_SFG.SFGINF_COMISIONDIFERENCIAL_GetBaseReportData(@p_CODCICLOFACTURACIONPDV NUMERIC(22,0),
  ...
  BEGIN
    ...
    SET @strSQLINSTRUCT = 'SELECT CAST(CODIGOAGRUPACIONGTECH NUMERIC(38,0)) AS CODIGO, CONVERT(VARCHAR,PLT.NOMPLANTILLAPRODUCTO) AS PLANTILLA, CONVERT(VARCHAR,AGR.NOMAGRUPACIONPUNTODEVENTA) AS CADENA';
    ...
    SET @strWHERECLAUSE = 'WHERE PLT.CODLINEADENEGOCIO = ' + ISNULL(@p_CODLINEADENEGOCIO, '') + ' ' +
    ...
    SET @sql = 
      ...
      'LEFT OUTER JOIN WSXML_SFG.PLANTILLAPRODUCTODETALLE PLD ON (PLD.CODPLANTILLAPRODUCTO = PLT.ID_PLANTILLAPRODUCTO) ' +
      'LEFT OUTER JOIN WSXML_SFG.RANGOCOMISION RCM ON (PLD.CODRANGOCOMISION = RCM.ID_RANGOCOMISION) ' +
      'WHERE PLT.CODLINEADENEGOCIO = ' + ISNULL(@p_CODLINEADENEGOCIO, '') + ' ' +
...
DECLARE @tPRODUCT__ID_PRODUCTO NUMERIC(38,0), @tPRODUCT__CODIGOGTECHPRODUCTO  NUMERIC(38,0), @tPRODUCT__NOMPRODUCTO VARCHAR(255)
 ...
 BEGIN
    ...
    SET @strSQLINSTRUCT = 'SELECT CAST(AGR.CODIGOAGRUPACIONGTECH NUMERIC(38,0)) AS CODIGO, CONVERT(VARCHAR,PLT.NOMPLANTILLAPRODUCTO) AS PLANTILLA, CONVERT(VARCHAR,AGR.NOMAGRUPACIONPUNTODEVENTA) AS CADENA';
    SET @strTFROMCLAUSE = 'FROM WSXML_SFG.PLANTILLAPRODUCTO PLT ' +
                      'INNER JOIN WSXML_SFG.AGRUPACIONPUNTODEVENTA AGR ON (AGR.ID_AGRUPACIONPUNTODEVENTA = PLT.CODAGRUPACIONPUNTODEVENTA) ' +
                      'LEFT OUTER JOIN WSXML_SFG. PLD ON (PLD.CODPLANTILLAPRODUCTO = PLT.ID_PLANTILLAPRODUCTO) ' +
                      'LEFT OUTER JOIN WSXML_SFG.VW_SHOW_PDVFACTURACION VPV ON (VPV.ID_CICLOFACTURACIONPDV = ' + ISNULL(@p_CODCICLOFACTURACIONPDV, '') + ' AND VPV.CODAGRUPACIONPUNTODEVENTA = PLT.CODAGRUPACIONPUNTODEVENTA AND VPV.ID_PRODUCTO = PLD.CODPRODUCTO) ';
    SET @strWHERECLAUSE = 'WHERE PLT.CODLINEADENEGOCIO = ' + ISNULL(@p_CODLINEADENEGOCIO, '') + ' ' +
                      'AND MASTERPLANTILLA = 0 ' +
    ...
    SET @sql = 
      ...
      'FROM WSXML_SFG.PLANTILLAPRODUCTO PLT ' +
      'LEFT OUTER JOIN WSXML_SFG.PLANTILLAPRODUCTODETALLE PLD ON (PLD.CODPLANTILLAPRODUCTO = PLT.ID_PLANTILLAPRODUCTO) ' +
      'LEFT OUTER JOIN WSXML_SFG.VW_SHOW_PDVFACTURACION VPV ON (VPV.ID_CICLOFACTURACIONPDV = ' + ISNULL(@p_CODCICLOFACTURACIONPDV, '') + ' AND VPV.ID_PRODUCTO = PLD.CODPRODUCTO AND VPV.CODAGRUPACIONPUNTODEVENTA = WSXML_SFG.AGRUPACION_F(0)) ' +
      'WHERE PLT.CODLINEADENEGOCIO = ' + ISNULL(@p_CODLINEADENEGOCIO, '') + ' ' +
      'AND MASTERPLANTILLA = 1 ' +
      'UNION ' +
      ISNULL(@strSQLINSTRUCT, '') + ISNULL(@strPRODCOLUMNS, '') + ' ' + ISNULL(@strTFROMCLAUSE, '') + ISNULL(@strWHERECLAUSE, '');
