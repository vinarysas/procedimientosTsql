...
CREATE PROCEDURE WSXML_SFG.SFGDETALLEPAGO_ReverseRecord(
 ...
 BEGIN
	...
	DECLARE @count_lstVINCULOS NUMERIC(22,0);
	
	SELECT @count_lstVINCULOS = COUNT(*)
	FROM WSXML_SFG.PAGOFACTURACIONPDV
    WHERE CODDETALLEPAGO = @pk_ID_DETALLEPAGO;
	
	IF @count_lstVINCULOS > 0 BEGIN
   ...
   -- Create Ctrl
    ...
    BEGIN
      ...
      IF @thisMAESTROFACTURACIONPDV > 0 BEGIN
		...
		IF @codPAGOFACTURACION IS NULL OR @codPAGOFACTURACION = 0 BEGIN
...
BEGIN
	...
	FROM WSXML_SFG.SFGSALDOPDV_GetCarteraSaldosActuales(@p_CODPUNTODEVENTA, @p_CODTIPOVINCULACIONPAGO);
    -- Saldar de acuerdo a pol!tica y verificar
    --SET @suggestedBALANCE = BALANCEPAYMENTLIST();
    DECLARE ic CURSOR FOR SELECT CODLINEADENEGOCIO, FIDUCIA, SALDOACTUAL FROM @availableBALANCE--.First..availableBALANCE.Last LOOP
	...
	WHILE @@FETCH_STATUS=0 BEGIN
		INSERT INTO @suggestedBALANCE VALUES ( @ic__CODLINEADENEGOCIO, @ic__FIDUCIA, @ic__SALDOACTUAL , 0, @ic__SALDOACTUAL );
		FETCH NEXT FROM ic INTO @ic__CODLINEADENEGOCIO, @ic__FIDUCIA, @ic__SALDOACTUAL 
    END;
	...
	DECLARE @rmPAGO FLOAT = @p_VALORPAGO;
	DECLARE @dstPAY FLOAT = 0;
	DECLARE @first__CODLINEADENEGOCIO NUMERIC(38,0) = 0;
    BEGIN
      -- Saldar.
      DECLARE ibsettle CURSOR FOR SELECT CODLINEADENEGOCIO, FIDUCIA, SALDOACTUAL FROM @availableBALANCE
	  OPEN ibsettle
	  
		DECLARE @ibsettle__CODLINEADENEGOCIO NUMERIC(38,0),@ibsettle__FIDUCIA NUMERIC(38,0), @ibsettle__SALDOACTUAL FLOAT
		
		FETCH NEXT FROM ibsettle INTO @ibsettle__CODLINEADENEGOCIO, @ibsettle__FIDUCIA, @ibsettle__SALDOACTUAL 
		
		WHILE @@FETCH_STATUS=0 BEGIN
	  
			-- Solo saldar si existe deuda
			IF @ibsettle__SALDOACTUAL  < 0 BEGIN
				IF @rmPAGO > ABS(@ibsettle__SALDOACTUAL) BEGIN
					UPDATE @suggestedBALANCE SET VALORPAGO = ABS(@ibsettle__SALDOACTUAL) WHERE CODLINEADENEGOCIO = @ibsettle__CODLINEADENEGOCIO;
					UPDATE @suggestedBALANCE SET NUEVOSALDO = 0 WHERE CODLINEADENEGOCIO = @ibsettle__CODLINEADENEGOCIO;
					SET @rmPAGO = @rmPAGO - ABS(@ibsettle__SALDOACTUAL);
				END ELSE BEGIN -- No saldar mas. Restar si queda.
					IF @rmPAGO > 0 BEGIN
					  UPDATE @suggestedBALANCE SET VALORPAGO = @rmPAGO WHERE CODLINEADENEGOCIO = @ibsettle__CODLINEADENEGOCIO;
					  UPDATE @suggestedBALANCE SET NUEVOSALDO = (@ibsettle__SALDOACTUAL + @rmPAGO) WHERE CODLINEADENEGOCIO = @ibsettle__CODLINEADENEGOCIO;
					  SET @rmPAGO = 0;
					END 
					BREAK;
				END
      ...
      -- Sumar a favor. Solo si tiene saldo a favor. Distribucion equitativa entre lineas de negocio
      IF @rmPAGO > 0 BEGIN
        IF @rmPAGO > (SELECT COUNT(*) FROM @availableBALANCE) BEGIN
          SET @dstPAY = ROUND(@rmPAGO / (SELECT COUNT(*) FROM @availableBALANCE),0);
          DECLARE isgst CURSOR FOR SELECT CODLINEADENEGOCIO, FIDUCIA,SALDOACTUAL, VALORPAGO, NUEVOSALDO FROM @suggestedBALANCE--.First..suggestedBALANCE.Last LOOP
		  OPEN isgst;
		  
		  DECLARE @isgst__CODLINEADENEGOCIO NUMERIC(38,0), @isgst__FIDUCIA NUMERIC(38,0), @isgst__SALDOACTUAL FLOAT, @isgst__VALORPAGO FLOAT, @isgst__NUEVOSALDO FLOAT
		  FETCH NEXT FROM isgst INTO @isgst__CODLINEADENEGOCIO, @isgst__FIDUCIA, @isgst__SALDOACTUAL, @isgst__VALORPAGO, @isgst__NUEVOSALDO
		  
		  DECLARE @fila NUMERIC(22,0) = 1
		  
		  WHILE @@FETCH_STATUS=0 BEGIN
		  
			IF @fila = 1 BEGIN
				SET @first__CODLINEADENEGOCIO = @isgst__CODLINEADENEGOCIO;
			END
			
            IF @rmPAGO > 0 BEGIN
				UPDATE @suggestedBALANCE SET VALORPAGO = (@isgst__VALORPAGO + @dstPAY) WHERE CODLINEADENEGOCIO = @isgst__CODLINEADENEGOCIO;
				UPDATE @suggestedBALANCE SET NUEVOSALDO = (@isgst__NUEVOSALDO + @dstPAY) WHERE CODLINEADENEGOCIO = @isgst__CODLINEADENEGOCIO;
				SET @rmPAGO = @rmPAGO - @dstPAY;
            END 
			SET @fila = @fila + 1
			FETCH NEXT FROM isgst INTO @isgst__CODLINEADENEGOCIO, @isgst__FIDUCIA, @isgst__SALDOACTUAL, @isgst__VALORPAGO, @isgst__NUEVOSALDO
          END;
          ...
          DEALLOCATE isgst;
        END 
        -- Si queda saldo, es por el redondeo. Adjuntarsela al primero
        IF @rmPAGO > 0 AND (SELECT COUNT(*) FROM @suggestedBALANCE) > 0 BEGIN
          -- Adjuntarsela al primero
		  UPDATE @suggestedBALANCE SET VALORPAGO = (VALORPAGO + @rmPAGO) WHERE CODLINEADENEGOCIO = @first__CODLINEADENEGOCIO;
		  UPDATE @suggestedBALANCE SET NUEVOSALDO = (NUEVOSALDO + @rmPAGO) WHERE CODLINEADENEGOCIO = @first__CODLINEADENEGOCIO;
        END 
      END 
	END
	SELECT CODLINEADENEGOCIO, FIDUCIA,SALDOACTUAL, VALORPAGO, NUEVOSALDO FROM @suggestedBALANCE;
END
GO
        ...
        BEGIN
			  ...
			  IF @@ROWCOUNT = 0 BEGIN
				...
				-- Todas las lineas de negocio posibles
					...
					DECLARE @ROWCOUNT1 NUMERIC(22,0) = 0
					DECLARE @ROWCOUNT2 NUMERIC(22,0) = 0
    ...
    BEGIN
        ...
        -- Determinar identificador de referencia, agrupamiento de tirilla y facturaciones de pdv
			BEGIN
				...
				IF @countBILLINGREFERENCES > 1 BEGIN
					GOTO MULTIPLEREFERENCE;
				END
		...
		BEGIN
				...
				IF (SELECT COUNT(*) FROM @lstLINEASDENEGOCIOTENTATIVO) > 0 BEGIN
					DECLARE ix CURSOR FOR SELECT CODLINEADENEGOCIO, FIDUCIA, SALDOACTUAL FROM @lstLINEASDENEGOCIOTENTATIVO;
					OPEN ix;
					...
					WHILE @@FETCH_STATUS=0
					BEGIN
						INSERT INTO WSXML_SFG.NOREFERENCIADOSUGERENCIA (/*ID_NOREFERENCIADOSUGERENCIA,*/ CODLINEADENEGOCIO, FIDUCIA, CODMEDIOPAGONOREF, CODUSUARIOMODIFICACION)
						VALUES (/*NOREFERENCIADOSUGERENCIA_SEQ.NextVal,*/
							  @ix__CODLINEADENEGOCIO,
							  @ix__FIDUCIA,
							  @codREFERENCIAN,
							  @p_CODUSUARIOMODIFICACION);
				...
				SET @errormessage = 'El valor del pago por ' + CONVERT(FLOAT,@p_VALORPAGO) + ' de referencia ' + @p_NUMEROREFERENCIA + ' cargado en el archivo ' + WSXML_SFG.ARCHIVONOMBRE_CTRLPAGO_F(@p_CODCTRLPAGO) + ' no corresponde con la facturacion. ';
				IF (SELECT COUNT(*) FROM @lstLINEASDENEGOCIOTENTATIVO) > 0 BEGIN
					SET @errormessage = @errormessage + 'Sin embargo, el valor coincide con la facturacion de ';
					
					FETCH NEXT FROM ix INTO  @ix__CODLINEADENEGOCIO, @ix__FIDUCIA, @ix__SALDOACTUAL
					WHILE @@FETCH_STATUS=0 BEGIN
						SET @errormessage = @errormessage + WSXML_SFG.LINEADENEGOCIO_NOMBRE_F(@ix__CODLINEADENEGOCIO) + ' ' + CASE WHEN @ix__FIDUCIA = 1 THEN '(Fiducia)' ELSE '(GTECH)' END + ', ';
					END
					CLOSE ix
					DEALLOCATE ix;
					SET @errormessage = @errormessage + '. Se espera aprobación.';
				END
				EXEC WSXML_SFG.SFGALERTA_GenerarAlerta @p_TIPOINFORMATIVO, 'VINCULAPAGOS', @errormessage, @p_CODUSUARIOMODIFICACION
...
CREATE PROCEDURE WSXML_SFG.SFGDETALLEPAGO_VincularPagos(@p_DETALLETAREAEJECUTADA NUMERIC(22,0), @p_RETVALUE_out NUMERIC(22,0) OUT) AS
 BEGIN
   ...
   SET NOCOUNT ON;
    ...
    IF (SELECT COUNT(*) FROM @lstPAYMENTS) > 0 BEGIN
	  ...
	  WHILE @@FETCH_STATUS=0 BEGIN
        ...
        BEGIN
          ...
          ELSE IF @cCODTIPOPAGO = @xCODTIPOPAGONOREFRNCIADO BEGIN
                  ...
                  IF (SELECT COUNT(*) FROM @lstBILLINGSTOLINK) > 0 BEGIN
					...
					WHILE @@FETCH_STATUS=0 BEGIN
                      ...
                      BEGIN
							...
							END TRY
							BEGIN CATCH
								SET @msg = '-20054 Error durante la vinculaci?n del pago no referenciado ' + ISNULL(WSXML_SFG.ORIGENPAGO_NOMBRE_F(@cCODORIGENPAGO), '') + ' por el valor de ' + ISNULL(CONVERT(VARCHAR,@cVALORPAGO), '') + ' no se pudo vincular: ' + isnull(ERROR_MESSAGE ( ), '')
								RAISERROR(@msg, 16, 1);
							END CATCH
                      END;
                  ...
                  IF @rmPAGO > 0 BEGIN
						...
						BEGIN TRY
						  ...
						  EXEC WSXML_SFG.SFGPAGOFACTURACIONPDV_AddRecord @cCODDETALLEPAGO,
														  ...
														  @cPAGOFACTURACION_out OUT
						  IF @@ROWCOUNT = 0 BEGIN
							SET @msg = '-20054 No se encontró facturación v lida para vincular el pago adelantado del punto ' + ISNULL(CONVERT(VARCHAR,WSXML_SFG.PUNTODEVENTA_CODIGO_F(@cCODPUNTODEVENTA)), '') + ' de tipo ' + ISNULL(WSXML_SFG.ORIGENPAGO_NOMBRE_F(@cCODORIGENPAGO), '') + ' por el valor de ' + ISNULL(CONVERT(VARCHAR,@cVALORPAGO), '') + ': ' + isnull(ERROR_MESSAGE ( ), '') 
							RAISERROR(@msg, 16, 1);
						  END
						...
						BEGIN CATCH
						  ...
						  SET @msg = '-20054 Ocurrió un error buscando una facturación para vincular el pago adelantado del punto ' + ISNULL(CONVERT(VARCHAR,WSXML_SFG.PUNTODEVENTA_CODIGO_F(@cCODPUNTODEVENTA)), '') + ' de tipo ' + ISNULL(WSXML_SFG.ORIGENPAGO_NOMBRE_F(@cCODORIGENPAGO), '') + ' por el valor de ' + ISNULL(CONVERT(VARCHAR,@cVALORPAGO), '') + ': ' + isnull(ERROR_MESSAGE ( ), '') 
						  RAISERROR(@msg, 16, 1);
						END CATCH
                    END;
                ...
                ELSE BEGIN
                  ...
                  BEGIN
					BEGIN TRY
						...
						IF @@ROWCOUNT = 0 BEGIN
							SET @msg = '-20054 No se encontró facturación v lida para vincular el pago adelantado del punto ' + ISNULL(CONVERT(VARCHAR,WSXML_SFG.PUNTODEVENTA_CODIGO_F(@cCODPUNTODEVENTA)), '') + ' de tipo ' + ISNULL(WSXML_SFG.ORIGENPAGO_NOMBRE_F(@cCODORIGENPAGO), '') + ' por el valor de ' + ISNULL(CONVERT(VARCHAR,@cVALORPAGO), '') + ': ' + isnull(@msg, '') 
							RAISERROR(@msg, 16, 1);
						END ELSE BEGIN
							EXEC WSXML_SFG.SFGPAGOFACTURACIONPDV_AddRecord @cCODDETALLEPAGO,
														@cCODMAESTROADELANTADO,
					...
					BEGIN CATCH
						-- Esa platica se perdio
						SET @msg = '-20054 Ocurrió un error buscando una facturación para vincular el pago adelantado del punto ' + ISNULL(CONVERT(VARCHAR,WSXML_SFG.PUNTODEVENTA_CODIGO_F(@cCODPUNTODEVENTA)), '') + ' de tipo ' + ISNULL(WSXML_SFG.ORIGENPAGO_NOMBRE_F(@cCODORIGENPAGO), '') + ' por el valor de ' + ISNULL(CONVERT(VARCHAR,@cVALORPAGO), '') + ': ' + isnull(@msg, '') 
						RAISERROR(@msg, 16, 1);
					END CATCH
                  END;
            ...
            END
            ELSE BEGIN
				SET @msg = 'El pago de tipo ' + ISNULL(WSXML_SFG.ORIGENPAGO_NOMBRE_F(@cCODORIGENPAGO), '') + ' por el valor de ' + ISNULL(CONVERT(VARCHAR,@cVALORPAGO), '') + ' no se puede vincular, ya que no tiene un punto de venta asociado'
    ...
    IF @countUNLINKEDPAYMENTS > 0 BEGIN
		SET @msg = 'Se ha terminado de vincular los pagos correctamente. Sin embargo, no se pudo encontrar vinculo para ' + ISNULL(CONVERT(VARCHAR,@countUNLINKEDPAYMENTS), '') + ' pagos.'
    ...
    END
    ELSE BEGIN
		EXEC  WSXML_SFG.SFGDETALLETAREAEJECUTADA_FinalizeExecution @p_DETALLETAREAEJECUTADA, 'Se ha terminado de vincular los pagos correctamente. Se logró encontrar un v!nculo para todos los pagos cargados.'
